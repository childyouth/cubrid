  ...
  if (attribute.header.name_space == ID_SHARED_ATTRIBUTE)
	  ...
	  || attribute.default_value.default_expr.default_expr_text != NULL)
	{
	  ...
	  if (attribute.default_value.default_expr.default_expr_text != NULL)
	    {
	      /* CBRD-26878: Expression-Derived Literal / residual / legacy whitelist DEFAULT --
	       * show the original expression, already in the parser's parenthesized normal
	       * form, e.g. "(1+1)".  The legacy DB_DEFAULT_EXPR_TYPE enum is no longer used. */
