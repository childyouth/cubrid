...
/* CBRD-26878: the column DEFAULT legacy "default_expr" enum is no longer
       * read; the DEFAULT is restored from the new properties below. */
...
/* CBRD-26878: the column DEFAULT legacy enum is no longer stored in the
       * "default_expr" property (the DEFAULT flows through the new properties
       * below).  Drop any stale legacy property so it is never persisted.  The
       * DB_DEFAULT_EXPR_TYPE enum survives only for ON UPDATE ("update_default")
       * and SHARED. */
